...
window.onload = () => {
    ...
    function showPicture() {
        if (document.getElementsByClassName("nameInput")[0].value != "") {
    ...
    function saveItem(event) {
        ...
        let name = document.getElementsByClassName("nameInput")[0];
        if (name.value != "") {
            localStorage.setItem('avatar', JSON.stringify(name.value));
            Swal.fire({
                icon: 'success',
                title: 'succes',
            ...
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: 'You have to fill in a username!'
            })
        }
    }
